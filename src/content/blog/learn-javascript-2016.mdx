...
you shouldn't _specifically_ learn Node.js. You should learn JavaScript. I mean
_real_ JavaScript. Not JavaScript, that little toy language you've used for
click handlers and the occasional HTTP request, but JavaScript, the language of
the Internet. _[Universal]_ (sometimes called _isomorphic_) JavaScript that runs
on the client _and_ the server without modification.
...
I actually still hear this one a lot. And... it's wrong. It's just _completely_
wrong. jQuery does very little to address problems with JavaScript as a
language. It does _a lot_ to smooth out the differences in APIs exposed by
browsers -- mostly the DOM (especially in older browsers, and _especially_ the
...
that the same _exact_ code that runs on the client also runs on the server. We
...
concept the authors refer to as a [virtual dom][vdom]. Essentially you _declare_
...
hybrid apps that run in a `webview`. I mean controlling _really real_ native
...
_everything_ updates, in-place. Talk about a productivity boost! This works
...
_progressive_ JavaScript framework. That is, a framework of micro-libraries you
...
identical to the [Flow] code above; however, in my experience, the tooling for
...
[Babel] can transform it. This post is intended to be more about the _why_ than
the _how_ so I'm going to leave it there for now. I might visit this topic again
in the future.

## Okay, I'm convinced. But _how_ do I learn JavaScript in 2016?
...
JavaScript has come a _long_ way since the early web days. If you haven't
...
<small>
  <sup>1, 2</sup> Or something like that.
</small>
